...
def print_conv_weights(model, ckpt_path):
    ...
    # # Find and print all Conv2D layer parameters
    # print_with_prefix("\n===== Conv2D Weights and Biases =====")
    
    # for name, param in clean_state_dict.items():
    #     # Check for convolution-related parameters
    #     if ('conv' in name.lower() or 'dwconv' in name.lower()):
    #         print_with_prefix(f"\n--- Layer: {name} ---")
    #         print_with_prefix(f"Shape: {param.shape}")
            
    #         # Check if weights are mostly zero
    #         zero_percentage = (param.abs() < 1e-6).float().mean().item() * 100
    #         if zero_percentage > 99:
    #             print_with_prefix(f"WARNING: {zero_percentage:.2f}% of values are near zero!")
            
    #         # Print statistics
    #         print_with_prefix(f"Min: {param.min().item():.6f}, Max: {param.max().item():.6f}")
    #         print_with_prefix(f"Mean: {param.mean().item():.6f}, Std: {param.std().item():.6f}")
            
    #         # Print actual values
    #         print_with_prefix("Values:")
    #         # For large tensors, only print a sample
    #         if param.numel() > 20:
    #             flat_param = param.flatten()
    #             print(flat_param[:10].tolist())  # Print first 10 elements
    #             print("... (truncated)")
    #         else:
    #             print(param.tolist())  # Print all elements for small tensors
...
if __name__ == "__main__":
    ...
    model = FlashDiT_models[train_config['model']['model_type']](
        ...
        # Print Conv2D weights if requested
    # if accelerator.process_index == 0:
    #     print_conv_weights(model, ckpt_dir)
    #     exit(0)  # Exit after printing weights if that's all we want
